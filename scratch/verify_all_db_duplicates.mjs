...
async function run() {
  try {
    ...
    console.log(`Analyzing pairwise duplicates among ${bookQuestions.length} questions...`);
    let dupCount = 0;
    for (let i = 0; i < bookQuestions.length; i++) {
      for (let j = i + 1; j < bookQuestions.length; j++) {
        if (isDuplicateQuestion(bookQuestions[i], bookQuestions[j])) {
          dupCount++;
          console.log(`\nDuplicate Pair #${dupCount}:`);
          console.log(`Q[${i + 1}] ID: ${bookQuestions[i].id} | ${bookQuestions[i].prompt.substring(0, 80)}...`);
          console.log(`Q[${j + 1}] ID: ${bookQuestions[j].id} | ${bookQuestions[j].prompt.substring(0, 80)}...`);
        }
      }
    }
    console.log(`\nTotal duplicate pairs found: ${dupCount}`);
  } catch (err) {
    console.error(err);
  }
}
