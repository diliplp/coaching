...
async function run() {
  try {
    ...
    console.log(`Analyzing ${bookQuestions.length} production questions...`);
    ...
    for (const q of bookQuestions) {
      ...
      for (const u of unique) {
        if (isDuplicateQuestion(u, q)) {
          duplicates.push({ q1: u, q2: q });
          found = true;
          break;
        }
      }
    ...
    console.log(`\nUnique Count (under exact logic): ${unique.length}`);
    console.log(`Duplicate Count (under exact logic): ${duplicates.length}`);
    
    if (duplicates.length > 0) {
      console.log("\nPairs of duplicates found in prod:");
      duplicates.forEach(({ q1, q2 }, idx) => {
        console.log(`\n[${idx + 1}] Duplicate:`);
