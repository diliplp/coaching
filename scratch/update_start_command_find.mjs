...
const startCommand = `node -e 'const fs = require("fs"); const glob = (dir) => { try { fs.readdirSync(dir, { withFileTypes: true }).forEach(d => { const res = require("path").resolve(dir, d.name); if (d.name === "node_modules" || d.name === ".git") return; if (d.isDirectory()) glob(res); else if (d.name === "pdf.js") console.log("FOUND PDFJS AT:", res); }); } catch(e){} }; glob(".");' && node backend/dist/index.js`;
